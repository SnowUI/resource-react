...
import { wrapSvgAsJsx, generateIconComponentCode, generateAvatarComponentCode, generateIllustrationComponentCode } from './utils/generate-component';
...
async function main() {
  // When running from resource-react/scripts/, __dirname is resource-react/scripts/
  // So we need to go up one level to get to resource-react/
  const reactPkgDir = path.resolve(__dirname, '..');
  const srcDir = path.join(reactPkgDir, 'src');
  
  // Try to find resource-base directory
  // 1. First, try monorepo structure: ../resource-base (from resource-react/ to resource-base)
  ...
  for (const icon of icons) {
    ...
    iconByNameImports.add(`import ${compName} from './${compName}';`);
    iconByNameEntries.push(`  '${icon.kebabName}': ${compName},`);
    expectedIconFiles.add(`${compName}.tsx`);
    generated++;
  }
  ...
  for (const group of materialGroups) {
    ...
    for (const exportLine of data.barrelExports) {
      // Convert from './ComponentName' to './avatars/ComponentName' etc.
      ...
      if (match) {
        ...
        if (!exportedNames.has(compName)) {
          exportLines.add(`export { default as ${compName} } from './${group.folder}/${compName}';`);
