...
export function generateAvatarComponentCode(
  ...
  const componentCode = [
    ...
    '',
    `export const ${componentName}: React.FC<AvatarProps> = ({ size = ${defaultSize}, width, height, ...rest }) => {`,
    `  const resolvedWidth = size ?? width ?? ${defaultSize};`,
    `  const resolvedHeight = size ?? height ?? ${defaultSize};`,
    hasSizes 
      ? `  // 根据 size 选择对应尺寸的图片，如果没有精确匹配则使用最接近的尺寸`
      : '',
    hasSizes
      ? `  const closestSize = findClosestSize(resolvedWidth, ${componentName}AvailableSizes);`
      : '',
    hasSizes
      ? `  const imageSrc = ${componentName}SizeMap[closestSize as keyof typeof ${componentName}SizeMap] ?? ${importIdentifier};`
      : '',
    hasSizes ? '' : `  const imageSrc = ${importIdentifier};`,
    `  return (`,
    `    <img`,
    hasSizes ? `      src={imageSrc}` : `      src={${importIdentifier}}`,
    `      alt="${componentName}"`,
    `      width={resolvedWidth}`,
...
export function generateIllustrationComponentCode(
  ...
  const componentCode = [
    ...
    '',
    `export const ${componentName}: React.FC<BackgroundProps> = ({ width = ${defaultWidth}, height, ...rest }) => {`,
    `  const resolvedWidth = width ?? ${defaultWidth};`,
    hasSizes 
      ? `  // 根据 width 选择对应宽度的图片，如果没有精确匹配则使用最接近的宽度`
      : '',
    hasSizes
      ? `  const closestWidth = findClosestWidth(resolvedWidth, ${componentName}AvailableWidths);`
      : '',
    hasSizes
      ? `  const imageSrc = ${componentName}WidthMap[closestWidth as keyof typeof ${componentName}WidthMap] ?? ${importIdentifier};`
      : '',
    hasSizes ? '' : `  const imageSrc = ${importIdentifier};`,
    `  return (`,
    `    <img`,
