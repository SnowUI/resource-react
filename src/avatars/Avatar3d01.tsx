...
const Avatar3d01AvailableSizes = [16, 20, 24, 28, 32, 40, 48, 56, 64, 80, 128, 256, 512] as const;
...
export const Avatar3d01: React.FC<AvatarProps> = ({ size = 32, width, height, ...rest }) => {
  const resolvedWidth = size ?? width ?? 32;
  const resolvedHeight = size ?? height ?? 32;
  // 根据 size 选择对应尺寸的图片，如果没有精确匹配则使用最接近的尺寸
  const closestSize = findClosestSize(resolvedWidth, Avatar3d01AvailableSizes);
  const imageSrc = Avatar3d01SizeMap[closestSize as keyof typeof Avatar3d01SizeMap] ?? Avatar3d01Src;
  return (
    <img
      src={imageSrc}
      alt="Avatar3d01"
