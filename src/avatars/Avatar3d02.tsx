...
const Avatar3d02AvailableSizes = [16, 20, 24, 28, 32, 40, 48, 56, 64, 80, 128, 256, 512] as const;
...
export const Avatar3d02: React.FC<AvatarProps> = ({ size = 32, width, height, ...rest }) => {
  const resolvedWidth = size ?? width ?? 32;
  const resolvedHeight = size ?? height ?? 32;
  // 根据 size 选择对应尺寸的图片，如果没有精确匹配则使用最接近的尺寸
  const closestSize = findClosestSize(resolvedWidth, Avatar3d02AvailableSizes);
  const imageSrc = Avatar3d02SizeMap[closestSize as keyof typeof Avatar3d02SizeMap] ?? Avatar3d02Src;
  return (
    <img
      src={imageSrc}
      alt="Avatar3d02"
