...
export const Avatar3d04: React.FC<AvatarProps> = ({ size = 32, width, height, ...rest }) => {
  const resolvedWidth = size ?? width ?? 32;
  const resolvedHeight = size ?? height ?? 32;
  // 根据 size 选择对应尺寸的图片，如果没有精确匹配则使用最接近的尺寸
  const closestSize = findClosestSize(resolvedWidth, Avatar3d04AvailableSizes);
  const imageSrc = Avatar3d04SizeMap[closestSize as keyof typeof Avatar3d04SizeMap] ?? Avatar3d04Src;
  return (
    <img
      src={imageSrc}
      alt="Avatar3d04"
