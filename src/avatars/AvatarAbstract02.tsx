...
export const AvatarAbstract02: React.FC<AvatarProps> = ({ size = 32, width, height, ...rest }) => {
  const resolvedWidth = size ?? width ?? 32;
  const resolvedHeight = size ?? height ?? 32;
  // 根据 size 选择对应尺寸的图片，如果没有精确匹配则使用最接近的尺寸
  const closestSize = findClosestSize(resolvedWidth, AvatarAbstract02AvailableSizes);
  const imageSrc = AvatarAbstract02SizeMap[closestSize as keyof typeof AvatarAbstract02SizeMap] ?? AvatarAbstract02Src;
  return (
    <img
      src={imageSrc}
