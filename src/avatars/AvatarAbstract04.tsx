...
export const AvatarAbstract04: React.FC<AvatarProps> = ({ size = 32, width, height, ...rest }) => {
  const resolvedWidth = size ?? width ?? 32;
  const resolvedHeight = size ?? height ?? 32;
  // 根据 size 选择对应尺寸的图片，如果没有精确匹配则使用最接近的尺寸
  const closestSize = findClosestSize(resolvedWidth, AvatarAbstract04AvailableSizes);
  const imageSrc = AvatarAbstract04SizeMap[closestSize as keyof typeof AvatarAbstract04SizeMap] ?? AvatarAbstract04Src;
  return (
    <img
      src={imageSrc}
