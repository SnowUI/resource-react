...
export const AvatarFemale05: React.FC<AvatarProps> = ({ size = 32, width, height, ...rest }) => {
  const resolvedWidth = size ?? width ?? 32;
  const resolvedHeight = size ?? height ?? 32;
  // 根据 size 选择对应尺寸的图片，如果没有精确匹配则使用最接近的尺寸
  const closestSize = findClosestSize(resolvedWidth, AvatarFemale05AvailableSizes);
  const imageSrc = AvatarFemale05SizeMap[closestSize as keyof typeof AvatarFemale05SizeMap] ?? AvatarFemale05Src;
  return (
    <img
      src={imageSrc}
      alt="AvatarFemale05"
